...
def _init_avanza():
    ...
    try:
        ...
        data = {
            ...
            "authentication_session": getattr(
                _AVANZA_INSTANCE, "_authentication_session", None
            ),
            "push_subscription_id": getattr(
                _AVANZA_INSTANCE, "_push_subscription_id", None
            ),
        ...
        logger.error(
            f"Failed to initialize Avanza: {e}. Sleeping 30s to prevent rapid restarts/TOTP reuse..."
        )
        time.sleep(30)
        raise e
